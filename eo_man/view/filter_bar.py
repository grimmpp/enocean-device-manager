...
class FilterBar:
    def __init__(self, main: Tk, app_bus: AppBus, data_manager: DataManager, row: int):
        ...
        self.cb_filtername.bind(
            "<Return>",
            lambda e: [
                self.load_filter(),
                self.add_filter(False),
                self.apply_filter(e, True),
            ],
        )
        ...
        self.cb_device_address.bind(
            "<KeyRelease>", lambda e: self.cb_filtername.set("")
        )
        ...
        self.app_bus.add_event_handler(
            AppBusEventType.SET_DATA_TABLE_FILTER, self.on_set_filter_handler
        )
        self.app_bus.add_event_handler(
            AppBusEventType.ADDED_DATA_TABLE_FILTER, self.on_filter_added_handler
        )
        self.app_bus.add_event_handler(
            AppBusEventType.REMOVED_DATA_TABLE_FILTER, self.on_filter_removed_handler
        )
        ...
        if (
            ...
            and len(selected_fn) > 0
            and selected_fn in self.data_manager.data_fitlers.keys()
        ):
    ...
    def on_filter_added_handler(self, filter: DataFilter):
        ...
        self.cb_filtername["values"] = sorted(
            [k for k in self.data_manager.data_fitlers.keys()]
        )
    ...
    def add_filter(self, show_error_message: bool = True):
        ...
        if filter_obj and len(filter_obj.name) > 1:
            ...
            messagebox.showerror(
                title="Error: Cannot add filter",
                message="Please, provdied a valid filter name!",
            )
    ...
    def remove_filter(self):
        ...
        if len(filter_obj.name) > 1:
            ...
            messagebox.showerror(
                title="Error: Cannot remove filter",
                message="Please, provdied a valid filter name!",
            )
    ...
    def set_widget_values(self, filter: DataFilter):
        if filter is not None:
            ...
            self.cb_external_address.insert(
                END, ", ".join(filter.device_external_address_filter)
            )
            self.select_ChecklistCombobox(
                self.cb_device_type, filter.device_type_filter
            )
    ...
    def select_ChecklistCombobox(self, widget: ChecklistCombobox, values: list[str]):
        ...
        for i in range(0, len(widget.checkbuttons)):
            widget.variables[i].set(
                1 if widget.checkbuttons[i].cget("text") in values else 0
            )
    ...
    def get_str_array(self, widget: Widget) -> list[str]:
        ...
        if isinstance(widget_value, str):
            return [
                g.strip()
                for g in widget_value.replace(";", ",").split(",")
                if len(g.strip()) > 0
            ]
