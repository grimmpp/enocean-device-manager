...
class StatusBar:
    def __init__(self, main: Tk, app_bus: AppBus, data_manager: DataManager, row: int):
        ...
        self.app_bus.add_event_handler(
            AppBusEventType.DEVICE_ITERATION_PROGRESS, self.device_scan_progress_handler
        )
        self.app_bus.add_event_handler(
            AppBusEventType.UPDATE_DEVICE_REPRESENTATION, self.update_device_count
        )
        self.app_bus.add_event_handler(
            AppBusEventType.UPDATE_SENSOR_REPRESENTATION, self.update_device_count
        )
        self.app_bus.add_event_handler(
            AppBusEventType.DEVICE_SCAN_STATUS, self.device_scan_status_handler
        )
        self.app_bus.add_event_handler(
            AppBusEventType.SET_DATA_TABLE_FILTER, self.active_filter_name_handler
        )

        ## initialize
        if self.data_manager.selected_data_filter_name is not None:
            self.active_filter_name_handler(
    ...
    def get_device_count_str(self) -> str:
        ...
        fam14s: int = len(
            [d for d in self.data_manager.devices.values() if d.is_fam14()]
        )
        bus_device: int = len(
            [d for d in self.data_manager.devices.values() if d.bus_device]
        )
