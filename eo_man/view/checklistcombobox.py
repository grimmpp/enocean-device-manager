...
class ChecklistCombobox(ttk.Combobox):
    ...
    def __init__(self, master=None, **kw):
        ...
        self.tk.eval(
            "ttk::combobox::ConfigureListbox %s" % (self)
        )  # This updates the listbox in the popdown
        ...
        self.canvas_frame = tk.Frame(
            self.popdown_frame
        )  # Frame in front of canvas for borders
        ...
        self.listbox.configure(
            yscrollcommand=""
        )  # Break connection between listbox and scrollbar
        ...
        self.listbox.bind(
            "<<PrevWindow>>", self.on_lb_prevwindow
        )  # Relates to the Tab key
        self.listbox.bind("<MouseWheel>", self.do_nothing)
        # self.listbox.bind("<Map>",self.do_nothing) # This almost works
        self.bind(
            "<MouseWheel>", self.do_nothing
        )  # MouseWheel on Entry widget in this case is nonsensical
        if self.tk.eval(
        ...
        self.bind(
            "<ButtonPress-1>", lambda event: self.popdown.focus_set()
        )  # Don't let the focus get set on the entry widget before mapping, to avoid "flickering"
    ...
    def on_lb_tab(self, event):
        self.on_checkbutton_click_release(
            event
        )  # Simulate the pressing of the currently selected button
        self.tk.eval(
    ...
    def on_motion(self, event):
        if event.widget in [str(b) for b in self.checkbuttons]:  # It was a button
            for (
                button
            ) in self.checkbuttons:  # Check all the buttons to see if we are inside one
    ...
    def configure_popdown(self, event=None):
        ...
        if len(self.cget("values")) > self.cget(
            "height"
        ):  # If we will have a scrollbar
    ...
    def popdown_unmap(self, event):
        if event.widget != self.popdown._w:
            return  # Only if the widget is the popdown
        self.autoscrolling = False
        self.mouse_has_entered_popdown = False
    ...
    def scroll_to_last_clicked_button(self, event):
        ...
        if event.widget != self.popdown_frame._w:
            return  # Only if the widget is the popdown_frame
        if not self.scrollbar.winfo_ismapped():
            return  # Only if the scrollbar is mapped
        ...
        if dt >= 0 and db < 0:  # Gotta scroll the view down
            if (
                db >= -2
            ):  # If the bottom button is > 2 buttons away from the last clicked button
            ...
            if (
                dt >= -2
            ):  # If the top button is > 2 buttons away from the last clicked button
