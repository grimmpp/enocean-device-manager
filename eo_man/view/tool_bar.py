...
class ToolBar:
    ...
    def __init__(self, main: Tk, menu_presenter: MenuPresenter, row: int):
        ...
        b = self._create_img_button(
            f,
            "Save to current file",
            ImageGallery.get_save_file_icon(),
            menu_presenter.save_file,
        )
        b = self._create_img_button(
            f,
            "Save as file",
            ImageGallery.get_save_file_as_icon(),
            lambda: menu_presenter.save_file(save_as=True),
        )
        b = self._create_img_button(
            f,
            "Open file",
            ImageGallery.get_open_folder_icon(),
            menu_presenter.load_file,
        )
        ...
        b = self._create_img_button(
            ...
            ImageGallery.get_ha_logo(),
            menu_presenter.export_ha_config,
        )
        ...
        b = self._create_img_button(
            f,
            "Send Message",
            ImageGallery.get_forward_mail(),
            menu_presenter.show_send_message_window,
        )

        # placed at the right end
        b = DonationButton(f, relief=GROOVE, small_icon=True).pack(
            side=RIGHT, padx=(0, 2), pady=2
        )
        b = self._create_img_button(
            f,
            "GitHub: EnOcean Device Manager Repository",
            ImageGallery.get_github_icon(),
            menu_presenter.open_eo_man_repo,
        )
        b.pack(side=RIGHT, padx=(0, 2), pady=2)
        b = self._create_img_button(
            f,
            "GitHub: EnOcean Device Manager Documentation",
            ImageGallery.get_help_icon(),
            menu_presenter.open_eo_man_documentation,
        )
        b.pack(side=RIGHT, padx=(0, 2), pady=2)
        ...
        if not AppInfo.is_version_up_to_date():
            ...
            b = self._create_img_button(
                f,
                f"New Software Version 'v{new_v}' is available.",
                ImageGallery.get_software_update_available_icon(),
                self.show_how_to_update,
            )
            b.pack(side=RIGHT, padx=(0, 2), pady=2)
    ...
    def _create_img_button(
        self, f: Frame, tooltip: str, image: ImageTk.PhotoImage, command
    ) -> Button:
        b = ttk.Button(
            f, image=image, cursor="hand2", command=command
        )  # , relief=GROOVE)
