...
class MenuPresenter:
    def __init__(
        ...
        file_menu.add_separator()
        file_menu.add_command(
            label="Remove all Device from Table",
        ...
        file_menu.add_command(
            label="Export Home Assistant Configuration",
            image=ha_icon,
            compound=LEFT,
        ...
        file_menu.add_command(
            label="Export Home Assistant Configuration as ...",
            image=ha_icon,
            compound=LEFT,
        ...
        menu_bar.add_cascade(menu=file_menu, label="File", accelerator="ALT+F")

        ha_menu = Menu(menu_bar, tearoff=False)
        menu_bar.add_cascade(label="Home Assistant", menu=ha_menu)
        ha_menu.add_command(
            label="Reset to suggested HA properties.",
        ...
        tool_menu.add_command(
            label="EEP Checker", command=lambda: EepCheckerWindow(main)
        )
        tool_menu.add_command(
            label="Send Message",
        ...
        tool_menu.add_command(
            label="Message Log Analyser",
            command=lambda: messagebox.showinfo(
                "Message Log Analyser", "Will be available soon!"
            ),
        )
        ...
        help_menu.add_cascade(menu=docs_menu, label="Documentation")
        docs_menu.add_command(
            label="EnOcean Device Manager ...",
            image=github_icon,
            compound=LEFT,
        ...
        docs_menu.add_separator()
        docs_menu.add_command(
            label="Home Assistant Eltako Integration ...",
            image=github_icon,
            compound=LEFT,
            command=lambda: webbrowser.open_new(
                r"https://github.com/grimmpp/home-assistant-eltako/tree/main/docs"
            ),
        )

        repos_menu = Menu(help_menu, tearoff=0)
        help_menu.add_cascade(menu=repos_menu, label="GitHub Repositories")
        repos_menu.add_command(
            label="EnOcean Device Manager ...",
            image=github_icon,
            compound=LEFT,
            command=lambda: webbrowser.open_new(
                r"https://github.com/grimmpp/enocean-device-manager"
            ),
        )
        ...
        repos_menu.add_separator()
        repos_menu.add_command(
            label="Eltako14Bus Communication Library ...",
            image=github_icon,
            compound=LEFT,
            command=lambda: webbrowser.open_new(
                r"https://github.com/grimmpp/eltako14bus"
            ),
        )
        ...
        try:
            if save_as or not os.path.isfile(self.remember_latest_filename):
                ...
                filename = filedialog.asksaveasfilename(
                    initialdir=self.remember_latest_filename,
                    title="Save Application State",
                    filetypes=[
                        ("EnOcean Device Manager", "*.eodm"),
                        ("EnOcean Device Manager", "*.yaml"),
                    ],
            ...
            self.data_manager.write_application_data_to_file(
                self.remember_latest_filename
            )
            ...
            self.main.title(
                f"{DEFAULT_WINDOW_TITLE} ({os.path.basename(self.remember_latest_filename)})"
            )
            self.app_bus.fire_event(
                AppBusEventType.LOG_MESSAGE,
                {
                    "msg": f"Save to File: '{self.remember_latest_filename}'",
                    "color": "red",
                },
            )
    ...
    def load_file(self, filename: str = None):
        ...
        if filename:
            ...
            self.app_bus.fire_event(
                AppBusEventType.LOG_MESSAGE,
                {
                    "msg": f"Load File: '{self.remember_latest_filename}'",
                    "color": "red",
                },
            )
    ...
    def export_ha_config(self, save_as: bool = False):
        try:
            ...
            self.app_bus.fire_event(
                AppBusEventType.LOG_MESSAGE,
                {"msg": msg, "color": "red", "log-level": "INFO"},
            )
            ...
            if error is not None:
                self.app_bus.fire_event(
                    AppBusEventType.LOG_MESSAGE,
                    {"msg": error, "log-level": "ERROR", "color": "red"},
                )
                if not messagebox.askyesno(
                    title="Error in Checking Configuration Data!",
                    message=error + "\n\nDo you want to try to continue?",
                ):
                    self.app_bus.fire_event(
                        AppBusEventType.LOG_MESSAGE,
                        {
                            "msg": "Exporting HA Configuration cancelled by user.",
                            "log-level": "INFO",
                        },
                    )
                    return

            self.ha_conf_gen.save_as_yaml_to_file(
    ...
    def reset_to_suggested_ha_properties(self):
        yes = messagebox.askyesno(
            "Reset to suggested HA properties.",
            "Do you want to reset Home Assistant relevant properties of all devices to initial values?",
        )
        if yes:
            for d in self.data_manager.devices.values():
                ...
                if d.is_bus_device():
                    ...
                    for me in d.memory_entries:
                        s = Device.get_decentralized_device_by_sensor_info(
                            me, d.base_id
                        )
                        if s.external_id in self.data_manager.devices:
                            Device.merge_devices(
                                self.data_manager.devices[s.external_id], s
                            )
                            self.app_bus.fire_event(
                                AppBusEventType.UPDATE_SENSOR_REPRESENTATION, s
                            )
    ...
    def open_eo_man_documentation(self):
        webbrowser.open_new(
            r"https://github.com/grimmpp/enocean-device-manager/tree/main/docs"
        )

    def show_send_message_window(self):
        if self.send_message_window is None:
            self.send_message_window = SendMessageWindow(
                self.main, self.app_bus, self.data_manager, self.serial_controller
            )
    ...
    def extend_pct14_export(self):
        ...
        def load():
            try:
                ...
                asyncio.run(
                    PCT14DataManager.write_sender_ids_into_existing_pct14_export(
                        source_filename,
                        target_filename,
