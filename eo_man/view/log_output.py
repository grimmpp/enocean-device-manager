...
class LogOutputPanel:
    def __init__(self, main: Tk, app_bus: AppBus, data_manager: DataManager):
        ...
        cb_show_values = ttk.Checkbutton(
            tool_bar, text="Show Telegram Values", variable=self.show_telegram_values
        )
        cb_show_values.pack(side=LEFT, padx=(2, 0))

        self.show_esp2_binary = tk.BooleanVar(value=False)
        cb_show_es2_binary = ttk.Checkbutton(
            tool_bar, text="Show ESP2 Binary", variable=self.show_esp2_binary
        )
        cb_show_es2_binary.pack(side=LEFT, padx=(2, 0))

        self.show_esp3_binary = tk.BooleanVar(value=False)
        cb_show_es3_binary = ttk.Checkbutton(
            tool_bar, text="Show ESP3 Binary", variable=self.show_esp3_binary
        )
    ...
    def serial_callback(self, data: dict):
        ...
        try:
            filter &= int.from_bytes(telegram.address, "big") > 0 and int.from_bytes(
                telegram.payload, "big"
            )
        except:
            pass
        ...
        if filter:
            ...
            adr = (
                str(telegram.address)
                if isinstance(telegram.address, int)
                else b2s(telegram.address)
            )
            ...
            values = ""
            eep, values = self.data_manager.get_values_from_message_to_string(
                telegram, current_base_id
            )
            if eep is not None:
            ...
            display_esp2: str = (
                f", ESP2: {telegram.serialize().hex()}"
                if self.show_esp2_binary.get()
                else ""
            )
            display_esp3: str = (
                f", ESP3: {''.join(f'{num:02x}' for num in ESP3SerialCommunicator.convert_esp2_to_esp3_message(telegram).build())}"
                if self.show_esp3_binary.get()
                else ""
            )
    ...
    def receive_log_message(self, data):
        ...
        if color:
            ...
            self.st.tag_add(
                "mark_" + color, f"{num_of_lines}.0", f"{num_of_lines}.{len(msg)}"
            )
