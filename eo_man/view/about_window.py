...
class AboutWindow:
    def __init__(self, main: Tk):
        ...
        l = tk.Label(
            popup, text="Version: " + AppInfo.get_version(), font="Arial 10", anchor="w"
        )
        l.pack(side=TOP, fill="x", pady=2, padx=5)

        self.get_label_link(
            text="PyPI: Package Name: " + __package__.split(".")[0],
            link=r"https://pypi.org/project/eo-man",
        )

        self.get_label_link(
            text="GitHub: grimmpp/enocean-device-manager", link=AppInfo.get_home_page()
        )

        self.get_label_link(
            text="GitHub: EnOcean Device Manager Documentation",
            link=r"https://github.com/grimmpp/enocean-device-manager/tree/main/docs",
        )

        self.get_label_link(
            text="GitHub: Report a bug or ask for features!",
            link=AppInfo.get_home_page() + "/issues",
        )

        self.get_label_link(
            text="Author: " + AppInfo.get_author(), link=r"https://github.com/grimmpp"
        )

        self.get_label_link(
            text="License - " + AppInfo.get_license(),
            link=r"https://github.com/grimmpp/enocean-device-manager/blob/main/LICENSE",
        )
