...
class SerialConnectionBar:
    def __init__(
        ...
        f.grid(
            row=row, column=0, columnspan=1, sticky=W + E + N + S, pady=(0, 2), padx=2
        )

        self.b_detect = ttk.Button(f, text="Detect Serial Ports")
        self.b_detect.pack(side=tk.LEFT, padx=(5, 5), pady=5)
        self.b_detect.config(
            command=lambda: self.detect_serial_ports_command(force_reload=True)
        )
        Hovertip(
            self.b_detect,
            "Serial port detection is sometime unstable. Please try again if device was not detected.",
            300,
        )
        ...
        self.cb_device_type["values"] = (
            get_display_names()
        )  # ['FAM14', 'FGW14-USB', 'FAM-USB', 'USB300', 'LAN Gateway']
        ...
        self.b_connect = ttk.Button(
            f,
            text="Connect",
            state=NORMAL,
            command=self.toggle_serial_connection_command,
        )
        ...
        self.b_scan = ttk.Button(
            f, text="Scan for devices", state=DISABLED, command=self.scan_for_devices
        )
        self.b_scan.pack(side=tk.LEFT, padx=(0, 5), pady=5)

        self.overwrite = tk.BooleanVar()
        self.cb_overwrite = ttk.Checkbutton(
            f, text="Overwrite existing values", variable=self.overwrite
        )
        ...
        self.b_sync_ha_sender = ttk.Button(
            f,
            text="Write to devices",
            state=DISABLED,
            command=self.write_ha_senders_to_devices,
        )
        ...
        self.app_bus.add_event_handler(
            AppBusEventType.CONNECTION_STATUS_CHANGE, self.is_connected_handler
        )
        self.app_bus.add_event_handler(
            AppBusEventType.DEVICE_SCAN_STATUS, self.device_scan_status_handler
        )
        self.app_bus.add_event_handler(
            AppBusEventType.WRITE_SENDER_IDS_TO_DEVICES_STATUS,
            self.device_scan_status_handler,
        )
        self.app_bus.add_event_handler(
            AppBusEventType.WINDOW_LOADED, self.on_window_loaded
        )
        self.app_bus.add_event_handler(
            AppBusEventType.UPDATE_DEVICE_REPRESENTATION, self.update_cb_gateways_for_HA
        )
        self.app_bus.add_event_handler(
            AppBusEventType.UPDATE_SENSOR_REPRESENTATION, self.update_cb_gateways_for_HA
        )
        self.app_bus.add_event_handler(
            AppBusEventType.SERVICE_ENDPOINTS_UPDATES, self.update_service_endpoints
        )
    ...
    def update_cb_gateways_for_HA(self, event=None):
        ...
        for d in self.data_manager.devices.values():
            if d.is_gateway():
                gateways.append(
                    f"{d.device_type.replace(' (Wireless Transceiver)', '')} ({d.external_id})"
                )
        ...
        for d in self.data_manager.devices.values():
            if d.base_id == self.serial_cntr.current_base_id and d.use_in_ha:
                if "sender" in d.additional_fields:
                    ...
                    sender_id = data_helper.a2i(sender_base_id) + data_helper.a2i(
                        d.address
                    )
                    sender_list[d.external_id]["sender"]["id"] = data_helper.a2s(
                        sender_id
                    )
    ...
    def device_scan_status_handler(self, status: str):
        if status == "FINISHED":
            self.is_connected_handler(
                {"connected": self.serial_cntr.is_serial_connection_active()}
            )
    ...
    def toggle_serial_connection_command(self):
        try:
            if not self.serial_cntr.is_serial_connection_active():
                ...
                self.b_connect.config(state=DISABLED)
                self.b_scan.config(state=DISABLED)
                self.serial_cntr.establish_serial_connection(
                    self.cb_serial_ports.get(), self.cb_device_type.get()
                )
            else:
            ...
            # reset buttons
            self.is_connected_handler(
                data={"connected": self.serial_cntr.is_serial_connection_active()}
            )
            LOGGER.exception("Was not able to detect serial ports.")

    def is_connected_handler(
        self, data: dict, skipp_serial_port_detection: bool = False
    ):
        ...
        if status:
            ...
            if (
                self.cb_device_type.get()
                == GATEWAY_DISPLAY_NAMES[GatewayDeviceType.EltakoFAM14]
                and self.serial_cntr.is_fam14_connection_active()
            ):
                self.b_scan.config(state=NORMAL)
                self.b_sync_ha_sender.config(state=NORMAL)
            else:
            ...
            if self.cb_device_type.get() in [
                GDN[GatewayDeviceType.LAN],
                GDN[GatewayDeviceType.LAN_ESP2],
            ]:
            ...
            self.b_scan.config(state=DISABLED)
            self.b_sync_ha_sender.config(state=DISABLED)
            if not skipp_serial_port_detection:
                self.detect_serial_ports_command()
    ...
    def update_combobox_serial_port(self):
        ...
        self.cb_device_type.config(state="readonly")
        try:
            self.cb_serial_ports["values"] = self.endpoint_list[
                get_gateway_type_by_name(self.cb_device_type.get())
            ]
    ...
    def detect_serial_ports_command(self, force_reload: bool = False):
        ...
        self.cb_serial_ports.config(state=DISABLED)
        self.app_bus.fire_event(
            AppBusEventType.REQUEST_SERVICE_ENDPOINT_DETECTION, force_reload
        )
