...
class DeviceDetails:
    def __init__(
        self, window: Tk, main: Tk, app_bus: AppBus, data_manager: DataManager
    ):
        self.window = window
        self.main = main
        ...
        self.app_bus.add_event_handler(
            AppBusEventType.SELECTED_DEVICE, self.selected_device_handler
        )
    ...
    def show_form(self, device: Device):
        ...
        self.text_ext_address.bind(
            "<Return>", lambda e, d=device: self.update_device(d)
        )
        ...
        device_info_btn = tk.Button(
            f2,
            relief=FLAT,
            borderwidth=0,
            image=ImageGallery.get_info_icon((16, 16)),
            cursor="hand2",
        )
        ...
        if device.is_gateway():
            ...
            self.cb_device_type.bind(
                "<Return>", lambda e, d=device: self.update_device(d)
            )
            self.cb_device_type.bind(
                "<<ComboboxSelected>>",
                lambda e, d=device: self.update_device(d, False, True),
            )
        ...
        cb = ttk.Checkbutton(
            f, text="Export to HA Config", variable=self.cb_export_ha_var
        )
        ...
        # additional fields
        c_row += 1
        c_row = self.add_additional_fields(
            device, device.additional_fields, f, "", c_row
        )
        ...
        self.btn_cancel = Button(
            f_btn, text="Reload", anchor=CENTER, command=self.reload_values
        )
        ...
        self.btn_apply = Button(
            ...
            text="Apply",
            anchor=CENTER,
            command=lambda d=device: self.update_device(d),
        )
    ...
    def update_device(
        ...
        suggest_default_values: bool = False,
    ):
        ...
        for key, value in add_fields.items():
            if not isinstance(value, dict):
                ...
                if key == "id" and parent_key == "sender":
                    ...
                    cb_s.bind(
                        "<Return>",
                        lambda e, af=add_fields, k=key, t=cb_s, d=device: [
                            set_additional_field_value(af, k, t),
                    ...
                    cb.bind(
                        "<Return>",
                        lambda e, af=add_fields, k=key, t=cb, d=device: [
                            set_additional_field_value(af, k, t),
                            self.update_device(d),
                        ],
                    )
                    ...
                    entry.bind(
                        ...
                        af=add_fields,
                        k=key,
                        t=entry: set_additional_field_value(af, k, t),
                    )  # set_additional_field_value(e,af,k,t))
                    entry.bind(
                        "<Return>",
                        lambda e, af=add_fields, k=key, t=entry, d=device: [
                            set_additional_field_value(af, k, t),
                            self.update_device(d),
                        ],
                    )
            ...
            if isinstance(value, dict):
                ...
                _row = self.add_additional_fields(
                    device, value, f, key, _row + 1, spaces + 3
                )
    ...
    def selected_device_handler(
        self, device: Device, force_update: bool = False
    ) -> None:
        ...
        if device is None:
            ...
            if (
                ...
                or force_update
                or self.current_device.external_id != device.external_id
            ):
