...
class DeviceTable:
    ...
    def __init__(self, main: Tk, app_bus: AppBus, data_manager: DataManager):
        ...
        def sort_rows_in_treeview(
            tree: ttk.Treeview, col_i: int, descending: bool, partent: str = ""
        ):
            data = [
                (tree.set(item, col_i), item) for item in tree.get_children(partent)
            ]
        ...
        def sort_treeview(tree: ttk.Treeview, col: int, descending: bool):
            ...
            tree.heading(
                i, command=lambda c=col, d=(not descending): sort_treeview(tree, c, d)
            )
        ...
        for col in columns:
            ...
            if col in ["Key Function"]:
                self.treeview.column(
                    i, anchor="w", width=250, minwidth=250
                )  # , stretch=NO)
            else:
                self.treeview.column(
                    i, anchor="w", width=80, minwidth=80
                )  # , stretch=NO)
            self.treeview.heading(
                i,
                text=col,
                anchor="center",
                command=lambda c=col, d=False: sort_treeview(self.treeview, c, d),
            )
        ...
        self.app_bus.add_event_handler(
            AppBusEventType.UPDATE_DEVICE_REPRESENTATION,
            self.update_device_representation_handler,
        )
        self.app_bus.add_event_handler(
            AppBusEventType.UPDATE_SENSOR_REPRESENTATION,
            self.update_sensor_representation_handler,
        )
        self.app_bus.add_event_handler(AppBusEventType.LOAD_FILE, self._reset)
        self.app_bus.add_event_handler(
            AppBusEventType.SET_DATA_TABLE_FILTER, self._set_data_filter_handler
        )
        self.app_bus.add_event_handler(
        ...
        if self.data_manager.selected_data_filter_name is not None:
            self._set_data_filter_handler(
                self.data_manager.data_fitlers[
                    self.data_manager.selected_data_filter_name
                ]
            )
    ...
    def update_device_handler(self, d: Device, parent: str = None):
        if (
            self.current_data_filter is not None
            and not self.current_data_filter.filter_device(d)
        ):
            return
        ...
        if not d.is_fam14():
            ...
            sender_adr = (
                ""
                if "sender" not in d.additional_fields
                else d.additional_fields["sender"][CONF_ID]
            )
            sender_eep = (
                ""
                if "sender" not in d.additional_fields
                else d.additional_fields["sender"][CONF_EEP]
            )
    ...
    def _serial_callback_handler(self, data: dict):
        ...
        if type(message) in [
            ...
            if not adr.startswith("00-00-00-"):
                ...
                d: Device = self.data_manager.find_device_by_local_address(
                    adr, current_base_id
                )
                if d is not None:
                    self.trigger_blinking(d.external_id)
