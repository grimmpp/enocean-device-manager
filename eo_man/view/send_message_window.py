...
class SendMessageWindow:
    ...
    def __init__(
        ...
        self.app_bus.add_event_handler(
            AppBusEventType.CONNECTION_STATUS_CHANGE, self.on_connection_state_changed
        )
    ...
    def show_window(self):
        ...
        for idx, cb_d in enumerate(
            [self.cb_data_0, self.cb_data_1, self.cb_data_2, self.cb_data_3]
        ):
        ...
        cb = ttk.Checkbutton(
            self.popup,
            text="Out-Going Telegram",
            command=self.show_message,
            variable=self.out_going_msg_boolvar,
        )
        ...
        self.f_telegram.grid(
            row=row, column=0, sticky=NSEW, columnspan=2, pady=_pady, padx=_padx
        )
        ...
        # send button
        self.b_send = ttk.Button(
            self.popup,
            text="Send",
            command=lambda send=True: self.show_message(None, send),
        )
        if not self.serial_controller.is_serial_connection_active():
            self.b_send.config(state=DISABLED)
        self.b_send.bind(
            "<Return>", lambda event, send=True: self.show_message(event, send)
        )
        self.b_send.grid(row=row, column=1, sticky=EW, padx=(0, 8))

        ttk.Separator(self.popup, orient=VERTICAL).grid(
            column=2, row=0, rowspan=10, sticky=NSEW
        )
        ...
        self.lb_fav_msg = ttk.Treeview(
            f,
            yscrollcommand=scrollbar.set,
            show="tree",
            height=4,
            selectmode="browse",
            style="SendMessageWindow.Treeview",
        )
        ...
        if self.data_manager.send_message_template_list:
            for history_entry in self.data_manager.send_message_template_list:
                ...
                if org == 7:
                    msg = Regular4BSMessage(address, status, data, outgoing)
                elif org == 6:
                    msg = Regular1BSMessage(address, status, data[0:1], outgoing)
                elif org == 5:
                    msg = RPSMessage(address, status, data[0:1], outgoing)
        ...
        self.b_add_fav = ttk.Button(
            f, text="Add as", command=lambda: self.add_fav_msg(self.current_telegram)
        )
        self.b_add_fav.bind(
            "<Return>", lambda e: self.add_fav_msg(self.current_telegram)
        )
    ...
    def send_update_of_current_message_templates(self):
        ...
        self.app_bus.fire_event(
            AppBusEventType.SEND_MESSAGE_TEMPLATE_LIST_UPDATED, entries
        )
    ...
    def add_fav_msg(self, msg: EltakoMessage, name: str = ""):
        if msg:
            ...
            text = ""
            if isinstance(msg, Regular4BSMessage):
                text += "4BS: "
            elif isinstance(msg, Regular1BSMessage):
                text += "1BS: "
            elif isinstance(msg, RPSMessage):
                text += "RPS: "
    ...
    def apply_fav_msg(self):
        if self.lb_fav_msg.selection():
            ...
            if "4BS" in item_text:
                self.cb_msg_type.set("4BS (Org = 0x07)")
            elif "1BS" in item_text:
                self.cb_msg_type.set("1BS (Org = 0x06)")
            elif "RPS" in item_text:
                self.cb_msg_type.set("RPS (Org = 0x05)")
    ...
    def set_sender_ids(self):
        ...
        if (
            self.serial_controller.current_base_id
            and not self.serial_controller.is_connected_gateway_device_bus()
        ):
            base_id = self.serial_controller.current_base_id
            ...
            self.cb_sender_id_3["values"] = [
                data_helper.a2s(i + int(base_id.split("-")[3]), 1)
            ...
            self.cb_sender_id_3["values"] = [
                data_helper.a2s(i, 1) for i in range(0, 256)
            ]
    ...
    def show_message(self, event=None, send: bool = False):
        ...
        try:
            ...
            data_str = f"{self.cb_data_0.get()}-{self.cb_data_1.get()}-{self.cb_data_2.get()}-{self.cb_data_3.get()}".replace(
                "---", "-00-00-00"
            )
            ...
            if "4BS" in msg_type:
                msg = Regular4BSMessage(
                    sender_id, status, data, self.out_going_msg_boolvar.get()
                )
            elif "1BS" in msg_type:
                data = data[0:1]
                msg = Regular1BSMessage(
                    sender_id, status, data, self.out_going_msg_boolvar.get()
                )
            ...
            if "4BS" not in msg_type:
                msg_text = msg_text[0:15] + msg_text[24:]
    ...
    def show_telegram_text(self, text: str, msg_type: str = ""):
        ...
        if "invalid" in text.lower():
            ...
            if "4BS" not in msg_type:
                data_end = 12 + 3
            l = ttk.Label(
                self.f_telegram, text=text[12:data_end], foreground=self.DATA_COLOR
            )
            l.pack(side=LEFT, anchor=NW)
            ...
            l = ttk.Label(
                self.f_telegram,
                text=text[data_end:sender_end],
                foreground=self.ADDRESS_COLOR,
            )
            l.pack(side=LEFT, anchor=NW)
            ...
            l = ttk.Label(
                self.f_telegram,
                text=text[sender_end : sender_end + 3],
            ...
            l = ttk.Label(
                self.f_telegram,
                text=text[sender_end + 3 : sender_end + 6],
                foreground="darkgrey",
            )
            l.pack(side=LEFT, anchor=NW)
