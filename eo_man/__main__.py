...
def cli_argument():
    p = argparse.ArgumentParser(
        description="""EnOcean Device Manager (https://github.com/grimmpp/enocean-device-manager) allows you to managed your EnOcean devices and to generate 
Home Assistant Configurations for the Home Assistant Eltako Integration (https://github.com/grimmpp/home-assistant-eltako)."""
    )
    p.add_argument(
        "-v", "--verbose", help="Logs all messages.", action="count", default=0
    )
    p.add_argument(
        "-c",
        "--app_config",
        help="Filename of stored application configuration. Filename must end with '.eodm'.",
        default=None,
    )
    p.add_argument(
        "-ha",
        "--ha_config",
        help="Filename for Home Assistant Configuration for Eltako Integration. By passing the filename it will disable the GUI and only generate the Home Assistant Configuration file.",
    )
    p.add_argument(
        "-pct14",
        "--pct14_export",
        help="Load PCT14 exported file. Filename must end with .xml",
    )
    return p.parse_args()
...
def init_logger(app_bus: AppBus, log_level: int = logging.INFO, verbose_level: int = 0):
    file_handler = RotatingFileHandler(
        os.path.join(PROJECT_DIR, "enocean-device-manager.log"),
        mode="a",
...
def main():
    ...
    if opts.app_config and opts.app_config.endswith(".eodm"):
        e = {
            "msg": f"Initially load data from file {opts.app_config}",
            "color": "darkred",
        }
        ...
        e = {
            "msg": f"Invalid filename {opts.app_config}. It must end with '.eodm'",
            "color": "darkred",
        }
        app_bus.fire_event(AppBusEventType.LOG_MESSAGE, e)
    elif opts.pct14_export and opts.pct14_export.endswith(".xml"):
        e = {
            "msg": f"Initially load exported data from PCT14 {opts.pct14_export}",
            "color": "darkred",
        }
        devices = asyncio.run(
            PCT14DataManager.get_devices_from_pct14(opts.pct14_export)
        )
    ...
    if opts.app_config and opts.app_config.endswith(".eodm") and opts.ha_config:
        HomeAssistantConfigurationGenerator(app_bus, data_manager).save_as_yaml_to_file(
            opts.ha_config
        )
    else:
