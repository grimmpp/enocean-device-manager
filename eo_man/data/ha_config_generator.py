...
"""
    ...
    def perform_tests(self) -> str:
        device_list = [
            ...
            if not d.is_gateway() and d.use_in_ha
        ]
        ...
        for d in device_list:
            if (
                ...
                if int(sender_id, 16) < 1 or int(sender_id, 16) > 127:
                    raise Exception(
                        f"sender id '{sender_id}' of device '{d.external_id}' is no valid number between 1 and 127."
                    )
                if sender_id in sender_ids:
                    raise Exception(
                        f"sender id '{sender_id}' is assigned more than once for at least device '{sender_ids[sender_id].external_id}' and '{d.external_id}'."
                    )
    ...
    def generate_ha_config(self, device_list: list[Device]) -> str:
        ha_platforms = set(
            [str(d.ha_platform) for d in device_list if d.ha_platform is not None]
        )
        ...
        for gw_d in gateways:
            ...
            if gw_d.device_type == GatewayDeviceType.LAN:
                out += (
                    f"    {CONF_GATEWAY_ADDRESS}: {gw_d.additional_fields['address']}\n"
                )
            out += f"    {CONF_DEVICES}:\n"
                ...
                if platform != "":
                    ...
                    for device in devices:
                        if device.ha_platform == platform:
                            # add devices
                            out += (
                                self.config_section_from_device_to_string(
                                    gw_d, device, True, 0
                                )
                                + "\n\n"
                            )
    ...
    def config_section_from_device_to_string(
        self, gateway: Device, device: Device, is_list: bool, space_count: int = 0
    ) -> str:
        out = ""
        spaces = space_count * " " + "        "
        ...
        if info and "PCT14-key-function" in info:
            ...
            out += (
                spaces[:-2]
                + f"  # Use 'Write HA senders to devices' button or enter manually sender id in PCT14 into function group {fg} with function {kf} \n"
            )
        adr = (
            device.address
            if gateway.is_wired_gateway() and gateway.base_id == device.base_id
            else device.external_id
        )
        out += spaces[:-2] + f"- {CONF_ID}: {adr.strip()}\n"
        out += spaces[:-2] + f"  {CONF_NAME}: {device.name}\n"
        out += spaces[:-2] + f"  {CONF_EEP}: {device.eep}\n"

        out += self.export_additional_fields(
            gateway, device.additional_fields, space_count
        )

        return out

    def export_additional_fields(
        self,
        gateway: Device,
        ...
        for key in additional_fields.keys():
            ...
            if parent_key in ["sender"] and key == "id":
                if gateway.is_wired_gateway():
                    sender_offset_id = self.LOCAL_SENDER_OFFSET_ID
                else:
                    sender_offset_id = gateway.base_id
                value = data_helper.a2s(
                    int("0x" + value[-2:], base=16) + data_helper.a2i(sender_offset_id)
                )
            if isinstance(value, str) or isinstance(value, int):
                ...
                out += f"{spaces}{key}: \n"
                out += self.export_additional_fields(
                    gateway, value, space_count + 2, key
                )
        return out
