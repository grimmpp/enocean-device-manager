    ...
    def get_app_info(cls, filename: str = None):
        if not cls.app_info:
            ...
            if pyproject_file:
                ...
                app_info["author"] = ", ".join(
                    [
                        f"{a.get('name')} {a.get('email', '')}".strip()
                        for a in pyproject_data["project"]["authors"]
                    ]
                )
            ...
            app_info["lastest_available_version"] = cls.pypi_info_latest_versino.get(
                "info", {}
            ).get("version", None)
            # get current/installed version
            if app_info["version"] is not None and app_info["version"] != "":
                cls.pypi_info_current_version = cls._get_info_from_pypi(
                    app_info["version"]
                )
