...
class GatewayRegistry:
    def __init__(self, app_bus: AppBus) -> None:
        ...
        self.lan_service_detector = LanServiceDetector(
            app_bus, self._update_lan_gateway_entries
        )
        ...
        self.app_bus.add_event_handler(
            AppBusEventType.REQUEST_SERVICE_ENDPOINT_DETECTION,
            self._process_update_service_endpoints,
        )
    ...
    def _update_lan_gateway_entries(self):
        self.endpoint_list[GDT.LAN.value] = (
            self.lan_service_detector.get_lan_gateway_endpoints()
        )
        self.endpoint_list[GDT.LAN_ESP2.value] = (
            self.lan_service_detector.get_virtual_network_gateway_service_endpoints()
        )

        self.app_bus.fire_event(
            AppBusEventType.SERVICE_ENDPOINTS_UPDATES, self.endpoint_list
        )

    async def async_update_service_endpoint_list(
        self, force_reload: bool = True
    ) -> None:
        if not force_reload and len(self.endpoint_list) > 0:
            ...
            self.endpoint_list: Dict[
                str, List[str]
            ] = await self.serial_port_detector.async_get_gateway2serial_port_mapping()
            self.endpoint_list[GDT.LAN.value] = (
                self.lan_service_detector.get_lan_gateway_endpoints()
            )
            self.endpoint_list[GDT.LAN_ESP2.value] = (
    ...
    async def _process_update_service_endpoints(self, force_update: bool = False):
        ...
        if not self._is_running.is_set():
            t = threading.Thread(
                target=process,
                name="Thread-async_update_service_endpoint_list",
                args=(force_update,),
            )
            t.daemon = True
            t.start()
