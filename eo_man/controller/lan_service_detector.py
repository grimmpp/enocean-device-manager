...
class LanServiceDetector:
    ...
    def add_service(self, zeroconf: Zeroconf, type, name):
        try:
            ...
            self.app_bus.fire_event(
                ...
                {"msg": msg, "log-level": "INFO", "color": "grey"},
            )

            for mdns_name in data_helper.MDNS_SERVICE_2_GW_TYPE_MAPPING:
                if mdns_name in name:
                    gw_type: GatewayDeviceType = (
                        data_helper.MDNS_SERVICE_2_GW_TYPE_MAPPING[mdns_name]
                    )
    ...
    def get_virtual_network_gateway_service_endpoints(self):
        return [
            f"{s['hostname'][:-1]}:{s['port']}"
            for s in self.service_reg_virt_lan_gw.values()
        ]
